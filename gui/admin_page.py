...
class AdminLoginFrame (tk.Frame) :
    ...
    def _build(self):
        label = tk.Label(self, text= "Admin Mode - Login", font=('Segoe UI', 14, 'bold'))
        label.pack(pady=8)

        frame = tk.Frame(self)
        frame.pack(pady=4)

        tk.Label(frame, text="Admin Token:").grid(row=0, column=0, sticky='e')
        self.token_var = tk.StringVar()
        tk.Entry(frame, textvariable=self.token_var, width=40, show='*').grid(row=0, column=1, padx=6)
...
class AdminDashboardFrame(tk.Frame):
    ...
    def _build(self):
        ...
        self.detail_label = tk.Label(right, text="Pilih user untuk lihat detail", justify='left')
    ...
    def _popup_rekening_baru(self):
        ...
        tk.Button(popup, text="Rekening Savings",
                  command=lambda: pilih_rekening(nomor_telepon_user, JenisRekening.SAVINGS)).pack(side='left', padx=10)
        tk.Button(popup, text="Rekening Checking",
                  command=lambda: pilih_rekening(nomor_telepon_user, JenisRekening.CHECKING)).pack(side='left', padx=10)
